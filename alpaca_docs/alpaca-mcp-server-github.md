# Alpaca MCP Server - Complete Documentation

## Overview

Alpaca's official MCP Server is a comprehensive Model Context Protocol (MCP) server that enables natural language trading operations through AI assistants like Claude, Cursor, and VS Code. It supports stocks, options, crypto, portfolio management, and real-time market data.
...
You need the following to configure and run the Alpaca MCP Server:
...
- **Claude Desktop** → Use `uvx` or `install.py` (Local)
- **Claude Mobile** → Deploy to cloud service (Remote Hosting)
- **ChatGPT** → Deploy to cloud service (Remote Hosting)
- **Cursor** → Use Cursor Directory or `install.py` (Local)
- **VS Code** → Use `uvx` (Local)
- **PyCharm** → Use `uvx` (Local)
- **Claude Code** → Use `uvx` or Docker (Local)
- **Gemini CLI** → Use `uvx` (Local)
...
2. **MCP client config JSON** (used by MCP client)
...
Update the API keys in the `env` section:
...
### Method 1: One-click installation with uvx from PyPI (Recommended)
...
**Note**: If you don't have `uv` yet, install it first and restart your terminal so `uv`/`uvx` are recognized. See: https://docs.astral.sh/uv/getting-started/installation/
...
After installing/cloning and activating the virtual environment:
...
01. What's my current account balance and buying power on Alpaca?
02. Show me my current positions in my Alpaca account.
03. Buy 5 shares of AAPL at market price.
...
09. Place a limit order to buy 100 shares of MSFT at $450.
...
**Note**: To access the latest 15-minute data, you need to subscribe to the [Algo Trader Plus Plan](https://alpaca.markets/data).
...
**Note**: At this moment, you can only view and update trading watchlists created via Alpaca's Trading API through the API itself.
...
**Note**: As of Nov 20, 2025, Alpaca does not provide a hosted Remote MCP Server. To use Alpaca's MCP Server on the Claude mobile app, you need to host it remotely on a cloud service, then connect it as a Connector on Claude desktop to access it from the mobile app.
...
For a simpler approach, visit the learn article "How to Deploy Alpaca's MCP Server Remotely on Claude Mobile App" where they demonstrate using [Render](https://render.com/) instead.

##### Method 2: Google Kubernetes Engine (GKE)

They also provide a [Helm chart](https://helm.sh/) under `alpaca-mcp-server/charts/alpaca-mcp-server` for deploying the Alpaca's MCP Server to [Kubernetes](https://kubernetes.io/) (Google Kubernetes Engine) as an example.
...
- Select "Add custom connector" and enter your preferred MCP Server name (e.g., Alpaca's MCP Server) and the URL `https://your-domain.com/mcp` in "Remote MCP Server URL" (ensure it ends with `/mcp`)
...
**Note**: As of Nov 20, 2025, Alpaca does not provide a hosted Remote MCP Server. To use Alpaca's MCP Server on the ChatGPT app, you need to host it remotely on a cloud service, then connect it as a Connector on ChatGPT web or mobile app to access it.

For more information, refer to the Alpaca learn article or ChatGPT's official documentation on Connectors.
...
Requires Docker installed and the image built locally (see [Docker Configuration](https://github.com/alpacahq/alpaca-mcp-server#docker-configuration)).
...
**Note**: Requires Node.js version 20 or higher.

#### 1. Install [Gemini-CLI](https://github.com/google-gemini/gemini-cli)

Authenticate yourself by login with Google, Gemini API Key, or Vertex AI depending on your purposes.
...
For more information, visit: https://github.com/google-gemini/gemini-cli/blob/main/docs/tools/mcp-server.md#how-to-set-up-your-mcp-server
...
**Topics**: options, crypto, trading, mcp, trading-api, stocks, algorithmic-trading, etfs, ai-trading, mcp-server

**Repository**: https://github.com/alpacahq/alpaca-mcp-server (450 stars, 131 forks)
