...
- [x] User switches account in dropdown
- [x] Next chat message uses NEW account's credentials
- [x] `mcp__alpaca__get_account_info` returns NEW account's data
- [x] No stale credential data after account switch

## Resolution (2026-02-01)

**Root Cause Identified:** The chat endpoint was using `demo-user` as a placeholder user ID instead of the authenticated user's ID. This caused credential validation to fail because credentials belong to the actual user, not `demo-user`.

**Fixes Applied:**

1. **Backend `main.py`** - Three endpoints fixed:
   - `/api/positions` - Added `user: AuthUser = Depends(get_current_user)`, changed `user_id = user.id`
   - `/api/orders` - Added `user: AuthUser = Depends(get_current_user)`, changed `user_id = user.id`
   - `/api/alpaca-agent/chat` - Added `user: AuthUser = Depends(get_current_user)`, changed `user_id = user.id`

2. **Frontend `AlpacaAgentChat.vue`** - Added `credentials: 'include'` to fetch call to send session cookies

**Verification:**
- Joe account: `api_key_fingerprint=...TYED`, Buying Power $1,313,270.80
- Muzz account: `api_key_fingerprint=...4D  `, Buying Power $154,171.55
- Credential switch confirmed working with different fingerprints per account
