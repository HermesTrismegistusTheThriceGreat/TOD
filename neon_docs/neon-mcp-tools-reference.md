...
Save the configuration file and restart or refresh your MCP client. An OAuth window will open in your browser. Follow the prompts to authorize your MCP client to access your Neon account.
...
#### Remote MCP Server Authentication with API Key
...
To prevent accidental modifications, enable read-only mode by adding the `x-read-only` header. This restricts the MCP server to only safe, non-destructive operations:
